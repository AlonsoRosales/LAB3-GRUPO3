...
public interface MascotaRepository extends JpaRepository<Mascota, Integer> {
    ...
    @Query(nativeQuery = true,value = "SELECT mascota.nombre as nombreMascota,anho as anho,hora_inicio as horaInicio,duracion " +
            "as duracion,entrega as entrega,responsable.nombre as responsable,opcion.descripcion as servicio " +
            "FROM servicio INNER JOIN responsable ON (responsable.idresponsable = servicio.responsable_idresponsable) " +
            "LEFT JOIN mascota ON (mascota.idmascota = servicio.mascota_idmascota) INNER JOIN opcion_servicio ON " +
            "(opcion_servicio.servicio_idservicio = servicio.idservicio) INNER JOIN opcion ON (opcion.idopcion = opcion_servicio.opcion_idopcion) WHERE idmascota = ?1")
    List<MascotasServicios> obtenerServiciosMascota(Integer id);
}